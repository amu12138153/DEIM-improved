...
class DEIMCriterion(nn.Module):
    ...
    def compute_main_losses(self, outputs, targets, epoch=0):
        """Compute comparable final-layer losses for train/validation curves.

        This method intentionally excludes decoder auxiliary, encoder auxiliary,
        denoising, pre-head and local/FGL losses.  Both training and validation
        therefore use the same loss definition:

            classification loss + L1 box loss + GIoU loss

        The returned values are already multiplied by ``self.weight_dict`` in
        exactly the same way as the normal training criterion.
        """
        required_keys = ("pred_logits", "pred_boxes")
        missing_keys = [key for key in required_keys if key not in outputs]
        if missing_keys:
            raise KeyError(
                "compute_main_losses requires model outputs containing "
                f"{required_keys}, but missing {missing_keys}."
            )
        ...
        for loss_name in self.losses:
            ...
            losses.update(weighted_loss_dict)

        if not losses:
            raise RuntimeError(
                "No comparable main losses were produced. "
                f"Configured losses: {self.losses}."
            )
