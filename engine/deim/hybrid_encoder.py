...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # channel projection 对每一层不同的通道数统一到256
        ...
        # encoder transformer 单层transformer
        ...
        ])#为每个需要 Transformer 增强的特征层级，独立创建一个 TransformerEncoder 实例
        #融合模块的输入
        ...
        self.fpn_blocks = nn.ModuleList()#融合模块
        for _ in range(len(in_channels) - 1, 0, -1):
            self.lateral_convs.append(copy.deepcopy(Lateral_Conv))
            self.fpn_blocks.append(copy.deepcopy(Fuse_Block))#融合模块

        # bottom-up pan 自底向上
        self.downsample_convs = nn.ModuleList()#下采样卷积
        self.pan_blocks = nn.ModuleList()#融合模块
    ...
    def _reset_parameters(self):
        if self.eval_spatial_size:
            for idx in self.use_encoder_idx:
                stride = self.feat_strides[idx]#每个特征层级的降采样倍数
    ...
    def forward(self, feats):
        ...
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]#统一通道
        ...
        # P5-lateral_convs-upsample_feat-concatP4-fpn_blocks-insert
        inner_outs = [proj_feats[-1]]
        for idx in range(len(self.in_channels) - 1, 0, -1):
            feat_heigh = inner_outs[0]#P5
            feat_low = proj_feats[idx - 1]#P4
            feat_heigh = self.lateral_convs[len(self.in_channels) - 1 - idx](feat_heigh)
            inner_outs[0] = feat_heigh
            upsample_feat = F.interpolate(feat_heigh, scale_factor=2., mode='nearest')#heighP4
            fused_feat = (upsample_feat + feat_low) \
                if self.fuse_op == 'sum' else torch.concat([upsample_feat, feat_low], dim=1)
                                                    #  concat   heighP4,P4
            inner_out = self.fpn_blocks[len(self.in_channels)-1-idx](fused_feat)#fpn_blocks
            inner_outs.insert(0, inner_out)#insert

        outs = [inner_outs[0]]
        #feat_lowP3-downsample_feat-concatP4-pan_blocks
