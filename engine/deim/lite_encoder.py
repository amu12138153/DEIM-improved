...
from ..mynewblock.Water_encoder_crossattn import *
import torch
...
class LiteEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if use_water_quality:
            self.water_fusion = WaterCrossAttentionforencoder(in_channels=hidden_dim)
    ...
    def forward(self, feats,water=None):
        ...
        fpn_feat = self.fpn_block(fuse_feat)

        if self.use_water_quality and water is not None:
            
            fpn_feat = self.water_fusion(fpn_feat, water)
            
        outs.append(fpn_feat)


        fuse_feat = proj_feats[1] + self.down_sample2(outs[-1])
        pan_feat = self.pan_block(fuse_feat)
        if self.use_water_quality and water is not None:

            pan_feat = self.water_fusion(pan_feat, water)
            

            
        outs.append(pan_feat)
