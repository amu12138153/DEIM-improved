...
class CocoDetection(torchvision.datasets.CocoDetection, DetDataset):
    ...
    def __init__(self, img_folder, ann_file, transforms, return_masks=False, remap_mscoco_category=False, sensor_csv=None, use_water_quality=False):
        ...
        self.water_dict = {}
        if self.use_water_quality and sensor_csv is not None:
            with open(self.sensor_csv, 'r', encoding='utf-8') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    file_name = os.path.basename(row["file_name"]) #是双引号
                    self.water_dict[file_name] = [
    ...
    def get_water_quality(self, idx):
        image_id = self.ids[idx]
        image_info = self.coco.loadImgs(image_id)[0]
        file_name = os.path.basename(image_info["file_name"])
        values = self.water_dict.get(file_name,None)
        if values is None:
            # 缺失时返回NaN，模型端经 nan_to_num 后等价于按均值填充
            values = [float("nan")] * 4
    ...
    def __getitem__(self, idx):
        ...
        if self.use_water_quality:
            target["water_quality"]= self.get_water_quality(idx)
        return img, target
