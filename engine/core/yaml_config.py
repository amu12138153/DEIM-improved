...
class YAMLConfig(BaseConfig):
    ...
    def get_optim_params(cfg: dict, model: nn.Module):
        ...
        duplicated = [name for name, count in visited_counter.items() if count > 1]
        missing = [name for name in names if name not in visited_counter]

        print("\n========== Optimizer Param Debug ==========")
        print("total trainable params:", len(names))
        print("visited params:", len(visited))
        ...
        assert len(duplicated) == 0 and len(missing) == 0, ''
        return param_groups
